...
def main():
    ...
    if uploaded_file:
        ...
        st.image(image, caption='업로드 된 이미지')
        st.write("분류 중...")
        ...
        response = requests.post("http://localhost:8001/results", files=files)
        label = response.json()["results"][0]["result"]
        st.write(f'분류된 라벨은 {label} 입니다.')
